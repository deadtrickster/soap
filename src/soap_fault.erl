...
-spec parse_fault_start(Version:: '1.1' | '1.2', any()) -> any().
parse_fault_start(Version, Model) ->
...
parse_fault_1_1({characters, Characters},
                _Namespaces,
                #pf_state{characters = String,
                         state = State} = S) when State =/= parsing_details ->
...
%% detail child start
parse_fault_1_1({startElement, Namespace, Tag, _, _} = Event,
                Namespaces,
                #pf_state{state = details,
                          model = Model} = S) ->

    Callback_state = erlsom_parse:new_state(Model, Namespaces),
    ...
    S#pf_state{state = parsing_details, parser_state = S2,
               parser = fun erlsom_parse:xml2StructCallback/2,
               detail_tag = {Tag, Namespace}};
parse_fault_1_1({endElement, _, "detail", _},
                _Namespaces,
                #pf_state{state = parsing_details,
                          parser = Parser,
                          parser_state = P_state,
                          details = _Details} = S) ->
    Parsed_fault = Parser(endDocument, P_state),
    S#pf_state{details = Parsed_fault,
               state = details_done};
parse_fault_1_1(Event,
                _Namespaces,
                #pf_state{state = parsing_details,
                          parser = Parser,
                          parser_state = P_state} = S) ->
    S#pf_state{parser_state = Parser(Event, P_state)};
%% detail child end
